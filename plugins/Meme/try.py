...
def main():
    meme = get_meme("guichu")
    d = ['left', 'right', 'top', 'bottom']
    for i in range(4):
        result = meme(images=['avatar.png'], texts=[], args={'direction': d[i]})
        # result = meme(images=[], texts=[], args={})

        with open(f"result{i}.gif", "wb") as f:
...
def generate_meme(
    ...
    for image in images:
        ...
        img = Path().cwd() / 'plugins/ImageDeal/images' / image
...
async def handle_message(bot: LegendWechatBot, msg: WxMsg):
    parser = construct_parser()
    result = parser.parse(msg.content)

    if not result.matched:
        if not isinstance(result.error_info, SpecialOptionTriggered):
            bot.sendMsg(result.error_info, msg.sender)
        return

    if not result.subcommands:
        bot.sendMsg(parser.get_help(), msg.sender)

    for subcommand, sub_result in result.subcommands.items():
        if subcommand == "list":
            bot.sendMsg(list_memes(), msg.sender)

        elif subcommand == "info":
            key = str(sub_result.args["key"])
            bot.sendMsg(meme_info(key), msg.sender)

        elif subcommand == "preview":
            key = str(sub_result.args["key"])
            bot.sendMsg(generate_meme_preview(key), msg.sender)
            ...
            for key, subsub_result in sub_result.subcommands.items():
                ...
                bot.sendMsg(generate_meme(key, images, texts, args), msg.sender)