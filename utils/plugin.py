...
class PluginManager:
    ...
    async def load_plugin(self, bot: Wcf, plugin_class: Type[PluginBase], is_disabled: bool = False) -> bool:
    ...
    async def load_plugins_from_directory(self, bot: Wcf, load_disabled_plugin: bool = True) -> Union[List[str], bool]:
        """从plugins目录批量加载插件"""
        loaded_plugins = []
    ...
    async def load_plugin_from_directory(self, bot: Wcf, plugin_name: str) -> bool:
        """从plugins目录加载单个插件
    ...
    async def reload_plugin(self, bot: Wcf, plugin_name: str) -> bool:
        """重载单个插件"""
    ...
    async def reload_all_plugins(self, bot: Wcf) -> List[str]:
        """重载所有插件
